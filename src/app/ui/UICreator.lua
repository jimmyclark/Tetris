...
--------------------------------------------基本图形--------------------------------------------------------------
function UICreator.createLine(parent,param,borderColor,fillColor,borderWidth)
	local x1 = param.x1;
	local y1 = param.y1;
	local x2 = param.x2 or (param.w - x1);
	local y2 = param.y2 or (param.h - y1);
	local params = {
					{x1,y1},
					{x2,y2},
				};
	local line = display.newPolygon(params,{fillColor=fillColor,borderWidth=20});
